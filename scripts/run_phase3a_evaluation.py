...
def main():
    ...
    parser.add_argument("--condition",choices=["base","A","AtoB"],required=True)
    ...
    adapter_path=Path(cfg["training"]["adapter_outputs"][str(args.seed)]["A" if args.condition=="A" else "B"]) if args.seed is not None else None
    ...
    if adapter_path:
        stage="A" if args.condition=="A" else "B"
        record_path=Path(cfg["training"]["record_outputs"][str(args.seed)][stage])
    ...
    for task in tasks:
        ...
        records.append(row)
        checkpoint.write_text(json.dumps({"phase":"3A","suite":args.suite,"condition":args.condition,"seed":args.seed,"completed_tasks":len(records),"records":records},indent=2)+"\n",encoding="utf-8")
