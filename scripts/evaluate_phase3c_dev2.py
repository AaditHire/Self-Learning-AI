"""Single greedy, no-documentation evaluation of one final DEV2 adapter."""
...
def main() -> None:
    ...
    if sha256(Path(__file__)) != preflight["execution_code_sha256"]["evaluate"]:
        raise ValueError("Evaluation execution code changed since preflight")
    if args.seed not in cfg["seeds"]:
        raise ValueError("Unregistered seed")
    root = ROOT / ".runtime/phase3c_dev2"
    ...
    if args.suite == "training":
        ...
        tasks = [{"task_id": row["example_id"], "family": row["family"],
                  ...
                  "difficulty": "development_training", "prompt": row["prompt"],
        ...
        tests = read("benchmark/phase3c_dev2/a_development_eval_hidden_tests.json")
        refs = {}
    expected_count = 60 if args.suite == "training" else 48
    ...
    for task in tasks:
        ...
        row = {"task_id": task["task_id"], "family": task["family"], "archetype": task["archetype"],
               "development_group": task["development_group"],
               "semantic_primitives": task["semantic_primitives"],
        ...
        checkpoint.write_text(json.dumps({"phase": "3C-DEV2", "condition": args.condition, "seed": args.seed,
    ...
    payload = {"phase": "3C-DEV2", "suite": args.suite, "condition": args.condition, "seed": args.seed,
