...
def main():
    ...
    if args.condition!="A":
        ...
        if not gate["all_eligible"]: raise RuntimeError("STOP_before_B_no_tuning: pre-B A acquisition failed")
    ...
    root=ROOT/cfg["runtime_root"]; out_dir=root/"adapters"/str(args.seed)/args.condition
    ...
    if parent:
        ...
        if prior.get("phase")!="3C" or prior.get("config_sha256")!=sha256(CONFIG): raise ValueError("A adapter provenance mismatch")
        adapter_cfg=json.loads((parent/"adapter_config.json").read_text(encoding="utf-8"))
        frozen_lora=cfg["training"]["lora"]
        if (adapter_cfg["r"]!=frozen_lora["rank"] or adapter_cfg["lora_alpha"]!=frozen_lora["alpha"]
                or adapter_cfg["lora_dropout"]!=frozen_lora["dropout"]
                or set(adapter_cfg["target_modules"])!=set(frozen_lora["target_modules"])):
            raise ValueError("A adapter LoRA configuration mismatch")
        other="replay" if args.condition=="naive" else "naive"
        other_record=root/"training"/str(args.seed)/f"{other}.json"
        if other_record.exists():
            other_record_payload=json.loads(other_record.read_text(encoding="utf-8"))
            if other_record_payload["lineage"]["parent_adapter_file_hashes"]!=parent_hashes:
                raise ValueError("Naive/replay branches have different A parents")
    a=items["a_train"]
    b=items["b_train"]
    ...
    train=cfg["training"]; system=(ROOT/"prompts/phase1t_system.txt").read_text(encoding="utf-8").strip()
    ...
    if other_record_payload and other_record_payload["training"]["trainable_parameters"]!=trainable:
        raise ValueError("Naive/replay trainable-parameter mismatch")
    ...
    if args.condition=="A":
        ...
        class RecordingSampler:
            def __iter__(self):
                epoch_ids=[]
                exposures.append(epoch_ids)
                for index in random_sampler:
                    epoch_ids.append(a[index]["example_id"])
                    yield index
            def __len__(self): return len(dataset)
        loader=DataLoader(dataset,batch_size=1,sampler=RecordingSampler(),generator=generator,collate_fn=collator(tokenizer.pad_token_id))
        epoch_loaders=[loader]*train["epochs"]
    else:
        schedule=items["schedule"]["by_seed"][str(args.seed)]
    ...
    if len(exposures)!=3 or any(len(ids)!=60 for ids in exposures): raise RuntimeError("Exposure log incomplete")
    if args.condition=="A" and any(set(ids)!={r["example_id"] for r in a} for ids in exposures):
        raise RuntimeError("A exposure order/log mismatch")
    ...
    record={"phase":"3C","condition":args.condition,"seed":args.seed,"config_sha256":sha256(CONFIG),"environment":{"platform":platform.platform(),"python":platform.python_version(),"torch":torch.__version__,"transformers":transformers.__version__,"bitsandbytes":bnb.__version__,"peft":peft.__version__,"cuda_runtime":torch.version.cuda,"gpu":torch.cuda.get_device_name(0)},"lineage":{"base_revision":model_cfg["revision"],"base_weight_hashes_before":base_before,"base_weight_hashes_after":base_after,"parent_adapter_path":str(parent).replace('\\','/') if parent else None,"parent_adapter_file_hashes":parent_hashes,"exposures_by_epoch":exposures},"data":{"examples_per_epoch":60,"a_examples_per_epoch":60 if args.condition=="A" else 0 if args.condition=="naive" else 12,"b_examples_per_epoch":60 if args.condition=="naive" else 48 if args.condition=="replay" else 0,"supervised_tokens":token_count},"training":{"epochs":train["epochs"],"optimizer_steps":step,"expected_optimizer_steps":expected,"load_seconds":load_seconds,"training_seconds":training_seconds,"peak_gpu_allocated_bytes":int(torch.cuda.max_memory_allocated()),"peak_process_rss_bytes":peak_rss,"trainable_parameters":trainable,"log":log},"adapter":{"path":str(out_dir).replace('\\','/'),"file_hashes":adapter_hashes}}
    record["lineage"]["schedule_sha256"]=sha256(ROOT/"research/protocols/phase3c_replay_schedule.json") if args.condition!="A" else None
    record["training"].update({"optimizer":train["optimizer"],"optimizer_initialization":"fresh_per_stage_and_branch",
                               "scheduler":train["scheduler"],"scheduler_initialization":"fresh_per_stage_and_branch",
                               "warmup_steps":train["warmup_steps"],"learning_rate":train["learning_rate"],
                               "micro_batch_size":train["micro_batch_size"],"gradient_accumulation_steps":accumulation,
                               "max_length":train["max_length"],"checkpoint_selection":train["checkpoint_selection"],
                               "rng_seed":args.seed})
