...
from self_learning_ai.benchmark import extract_source_phase1r, load_json, paired_comparison, score_source
...
def load_frozen_nf4(model_path: Path) -> Any:
...
def classify_failure(normalized: str, record: dict[str, Any]) -> str:
...
def summarize(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    def metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
        result: dict[str, Any] = {
            "n": len(rows),
            "failure_taxonomy": dict(sorted(Counter(row["failure_category"] for row in rows).items())),
        }
    ...
    return {
        ...
        "by_family": {family: metrics(rows) for family, rows in sorted(by_family.items())},
    }
...
def main() -> None:
    ...
    parser.add_argument("--model-key", choices=["qwen_3b", "qwen_1p5b_nf4"], required=True)
    ...
    for relative, expected in runtime["weight_file_sha256"].items():
        ...
        if actual != expected:
            raise ValueError(f"Model weight mismatch for {relative}: {actual}")

    tasks = load_json(Path(config["benchmark"]["tasks"]))
    hidden = load_json(Path(config["benchmark"]["hidden_tests"]))
    if len(tasks) != 64 or any(task["split"] != "phase1t_confirmation" for task in tasks):
        raise ValueError("Phase 1T benchmark invariant violated")
    condition_order = config["model_condition_order"][args.model_key]
    base_system = Path(config["prompt"]["system"]).read_text(encoding="utf-8").strip()
    user_template = Path(config["prompt"]["user_template"]).read_text(encoding="utf-8").strip()
    ...
    for context_file in config["prompt"]["candidate_c_context_files"]:
        ...
        docs_system += Path(context_file).read_text(encoding="utf-8").strip()
    ...
    model = load_frozen_nf4(model_path)
    ...
    for condition in condition_order:
        system = base_system if condition == "NO_DOCS" else docs_system
        records = []
        for task in tasks:
            user = user_template.format(task_id=task["task_id"], task_prompt=task["prompt"])
            ...
            if input_tokens > config["decoding"]["max_input_tokens"]:
                raise ValueError(f"Input budget exceeded for {condition}/{task['task_id']}")
            ...
            scored = score_source(compiler, task, hidden[task["task_id"]], normalized).as_dict()
            record = {
                "condition": condition,
            ...
            record["failure_category"] = classify_failure(normalized, record)
            records.append(record)
            checkpoint = {
                "model_key": args.model_key,
                "condition_order": condition_order,
                "completed_condition": condition,
                "completed_tasks_in_condition": len(records),
                "conditions": conditions | {condition: records},
            }
    ...
    payload: dict[str, Any] = {
        ...
        "summaries": {condition: summarize(records) for condition, records in conditions.items()},
    }
    if condition_order == ["NO_DOCS", "CANDIDATE_C"]:
        payload["paired_comparison"] = paired_comparison(
            conditions["NO_DOCS"],
            conditions["CANDIDATE_C"],
