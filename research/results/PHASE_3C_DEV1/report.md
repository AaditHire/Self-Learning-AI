...
The six finalized adapters, six raw training records, twelve raw evaluation records, preflight provenance, analysis, and adapter support files were byte-verified into the established Git/Git LFS preservation paths. The separate preservation manifest records original/destination paths, sizes, SHA-256, seed/condition, and local copy verification. Preservation commit `790bcc2a91cfb01a88a44e1468e84e0bc1418faa` was pushed normally to `origin/main`. A fresh separate remote clone retrieved the DEV1 LFS objects; **all 81 artifacts**, including all six adapter weights, matched their original size and SHA-256. Restored JSON parsed and adapter safetensors headers opened with tensor keys. The verification is `research/manifests/phase3c_dev1_remote_restore_verification.json`. Local originals remain unchanged under `.runtime/phase3c_dev1/`.
